use crate::{
	balance::Balance, entities::camera::{ARENA_HEIGHT, ARENA_WIDTH}, systems::{animation::Facing, movement::Walking}
};
use amethyst::{
...
pub fn initialize(world: &mut World, sprite_sheet: Handle<SpriteSheet>) {
	...
	let movement_parameters = world.read_resource::<Balance>().aliens.walking.clone();
	...
	for _ in 0..count {
		...
		world
			...
			.with(Walking::new(movement_parameters.clone()))
