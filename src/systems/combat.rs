...
use amethyst::{
	core::{SystemDesc, Time, Transform}, derive::SystemDesc, ecs::{
		Component, DenseVecStorage, Join, ReadExpect, ReadStorage, System, SystemData, World, WriteStorage
	}
};
		...
		for (fight, attacker_transform) in (&mut fights, &transforms).join() {
			...
			if fight.cooldown == 0.0 {
				if let Some(Attack) = &fight.attack {
					for (defender_transform, alien, life) in
						(&transforms, &aliens, &mut lives).join()
					{
